...
builds = requests.get('http://api.travis-ci.org/pouchdb/pouchdb/builds?event_type=push').json();

last_build_id = builds[0]['id'];

status = requests.get('http://api.travis-ci.org/pouchdb/pouchdb/builds/' + str(last_build_id)).json();
